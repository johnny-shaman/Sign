 ...
 * - 同一スコープ内での再定義は後勝ちで単純に上書きする。ただし**トップレベルは
 *   ここまで来ない**——`compile` が同じ名前の2つ目の定義を行ごと落とす（先勝ち、
 *   `refuseRedefinitions`）ので、表に届くのは最初の定義だけである。後勝ちが残って
 *   いるのはブロックの中だけで、そちらはシャドーイングと同じ話である。
