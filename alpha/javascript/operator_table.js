...
export const NOT_YET = {
  ...
  abs: "`cmp` + `cneg` などの短い列。**診断に名前が出ない**（`block` と言う）ので、節を足すときは `n.kind === \"abs\"` で拾う（`norm` と同じ形）",
};
 ...
 * 必要があり、それは `.sn` 側で 0 byte のスロットになって行の形を壊す（生成器の注記）。
 * 揃わない表は実行時の鍵で引けないので、**指令は指令の表を持つべきで、命令の表に混ぜない**。
 */
