...
// **TAB 1つが1段である**（利用者の決定 2026-09-15）。2段浅い続きの行は2段閉じてからつなぎ、今の深さより深い TAB は
// 揃えなので段を増やさない。どちらも印の数を1つ間違えるだけで括りの釣り合いが崩れる形だが、試す行が無かった。
check("2段浅い続きの行は先に2段閉じる", run("f : x ?\n\t\tx\n * 2\nf 3"), "6");
check("深い TAB の続きの行は段を増やさない", run("v :\n\t1\n\t\t\t+ 1\n\t5\nv"), JSON.stringify([2, 5]));
