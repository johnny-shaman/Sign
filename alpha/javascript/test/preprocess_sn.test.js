...
// 区切りの符号が CR なので、変換し損ねた CR は区切りに紛れて見えない。`\` の直後と、空白で始まらない次の行で
// 形を並べる（skip_lf を外しても、ここ以外は落ちなかった）。
same("\\ + CRLF の次の0列目の文字列", "t : `L1` \\\r\n`L2`");
same("\\ + CRLF の次の + 1", "a : \\\r\n+ 1");
same("\\ + 裸の CR", "c : \\\r\rc = 0u000A");
same("裸の CR だけで区切ったブロック", "f : x ?\r\tx = 1 : 10\r\t20\rf 2");
same("ブロックの最後の行が \\ + 改行", "f : x ?\n\tx = 1 : 10\n\t\\\n\nf 2");
same("\\\\ の後の改行は区切り", "c : \\\\\nc");
same("文字列の末尾の \\ の後の \\ + 改行", "s : `a\\` \\\n `b`\ns");
