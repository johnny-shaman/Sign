...
checkTrue("空リストの値は __", value("[]") === "__");
	...
	// **括りは要らない。** 器を撒いた後に引くのは断るが（撒いた後に器が残っていないので、
	// 引くには積を当て直す文脈が要る）、`String` は μ が強制なので撒いても器のままである
	// ——ここを一律に断ると、この法が書けなくなる。
	checkTrue("括らなくても同じ", value("s : `ab`\ns~ ' 1") === value("s : `ab`\ns ' 1"));
	checkTrue("名前に束ねても同じ", value("s : `ab`\nn : s~\nn ' 1") === value("s : `ab`\ns ' 1"));
	// **撒く動作も積む動作も、きちんと演算される**（利用者 2026-09-23「String でも……いわゆる
	// モナドになるわけだから」）。`s~ = s` が成り立つなら、`~` を書いたかどうかで**積んだ結果が
	// 変わってはならない**——両方撒いたときだけ吸収が外れており、`s t~` は \`abcde\` なのに
	// `s~ t~` は文字の列になっていた。`s~ = s` を代入しただけで値が変わる形である。
	const two = (e) => value("s : `abc`\nt : `de`\n" + e);
	checkTrue("両方撒いても積んだ結果は同じ", two("s~ t~") === two("s t"));
	checkTrue("片方だけ撒いても同じ", two("s t~") === two("s t") && two("s~ t") === two("s t"));
	checkTrue("文字列でない相手も吸収する", two("s~ 1") === two("s 1"));
	checkTrue("撒いて集め直しても同じ", two("[s~ t~]") === two("s t"));
	// 名前を通さない字面でも同じである（門は名前の型だけでなく字面も見る）。
	checkTrue("字面の文字列を撒いても引ける", value("`abc`~ ' 1") === value("`abc` ' 1"));
	checkTrue("括っても同じ", value("(`abc`~) ' 1") === value("`abc` ' 1"));
...
// ---- 余積のリスト化＝イテレータの生成（利用者 2026-09-24） ----
//
// `1 2 3` は列、`[1 ~ 3]` は規則（イテレータ）である。**余積で列を作ることと規則を作ることが
// 同じ**なら、この2つはどの演算から見ても区別できないはずである——表現の費用は違っても、
// それは同型の欠陥ではなく最適化である。
//
// **期待値は書かない。** 同じ列の2つの綴りが**同じ答え**になることだけを見る（両側が同じ
// 間違いをしていても落ちないが、片方の綴りだけ直った／壊れたときに落ちる軸である）。
// 実測では 18 形のうち 17 形が既に一致していて、割れていたのは**裸の rest へ撒いたとき**
// だけだった——規則が1つの実引数として渡り、`x` が列ごと飲んでいた。
{
	const AS = ["a : 1 2 3\n", "a : [1 ~ 3]\n"];
	const both = (note, expr) => {
		const l = value(AS[0] + expr);
		const r = value(AS[1] + expr);
		checkTrue(note, l === r, `余積=${l} / 規則=${r}`);
	};
	both("長さ", "||a||");
	both("添字", "a ' 0");
	both("切り出し", "a ' 1~");
