...
//
// `p~ [ zzz : 1 ]~` は「p の器へ入れる」形だが鍵が1本増える。p が自分の鍵ぶんしか場所を
// 持っていなければ入らないし、増える鍵が `aa` なら**名前順なので既存のスロットが全部
// ずれる**——少しずつ伸ばす方式（`ReDim Preserve`）は取れない。
//
// だが全プログラムを見るのだから、和集合はコンパイル時の事実である。定義の場所で和集合
// ぶんを一度だけ取れば、スロットの位置は途中で動かず、写しは一度も起きない。
{
	const MG = "p : [\n\tfoo : 10\n\tbar : 20\n]\n";
	check("和集合：後ろに増える鍵", slotsOf(MG + "q : p~ [\n\tzzz : 1\n]~"), ["bar@0", "foo@8", "zzz@16"]);
	// **ここがこの仕組みの核心。** `aa` は名前順で先頭に入るので、既存の2つが 8 byte ずつ
	// 後ろへ動く。動いても読む側は同じ表を引くので当たる——**両者が同じ表を引くこと**が、
	// 途中で伸ばせないことの答えである。
	check("和集合：前に増える鍵（既存が全部ずれる）", slotsOf(MG + "q : p~ [\n\taa : 1\n]~"), ["aa@0", "bar@8", "foo@16"]);
	check("和集合：大きさも和集合ぶん", sizeOf(MG + "q : p~ [\n\tzzz : 1\n]~"), 24);
	// 鍵が増えないマージは場所を変えない。**マージが書いてあることではなく、鍵が増える
	// ことが問い**である。
	check("和集合：重なる鍵だけなら伸びない", slotsOf(MG + "q : p~ [\n\tfoo : 99\n]~"), ["bar@0", "foo@8"]);
	// 何段でも和集合である。左に同じ名前を置くマージを全部集める。
	check("和集合：2つのマージが持ち込む鍵", slotsOf(MG + "q : p~ [\n\tzzz : 1\n]~\nr : p~ [\n\taa : 2\n]~"), ["aa@0", "bar@8", "foo@16", "zzz@24"]);
	// 撒く元は名前でもよい。持ち込む鍵はその器の並びから引く。
	const slotsOfNth = (source, n) => {
		const l = layoutOfStruct(compile(source).nodes[n].right, A64);
		return l && l.slots.map((s) => `${s.name}@${s.offset}`);
	};
	check("和集合：撒く元が名前", slotsOfNth("w : [\n\tzzz : 1\n]\n" + MG + "q : p~ w~", 1), ["bar@0", "foo@8", "zzz@16"]);
	// **`p~ p~` は鍵を増やさない。** 自分を撒く元として記録すると `layoutOfStruct` が自分の
	// 並びを待つ形になり、「決まらない」へ落ちる。
	check("和集合：自分を撒いても伸びない", slotsOf(MG + "q : p~ p~"), ["bar@0", "foo@8"]);
	// **連番（宣言順）は動かない。** 和集合で増えた鍵は自分の鍵の後ろに続く——`p ' 0` は
	// 宣言順の0番目であって、物理配置の0番目ではない。
	const ord = (src) => {
		const l = layoutOfStruct(rhs(src), A64);
		return l && l.slots.slice().sort((a, b) => a.ordinal - b.ordinal).map((s) => s.name);
	};
	check("和集合：連番は宣言順のまま", ord(MG + "q : p~ [\n\taa : 1\n]~"), ["foo", "bar", "aa"]);
}
