...
//
// 器として2本で受ける所まで揃っていても、要素が 1 byte（`String`）か `{ptr, len}`
// （`List(String)`）かで読む命令が変わる。合流でどちらかに決まった仮引数へもう片方を
// 渡すと、**呼び先は自分の読み方で読む**——診断ゼロで値だけが違っていた：
//
//     n0 : [~xs] ? ||xs ' 0||
//     via : ys ? n0 ys
//     w : n0 [\\a , \\b]
//     via [`ab` , `cd`]              解釈 2／実機 1（診断ゼロ）
//
// 同じ穴の深い形が `parser.sn` を括りの中へ降ろしたときで、`out` の仮引数が `tokens` の
// `List(String)` と合流して 16 byte になり、`_.main` が 3 byte で並べた並びをそのまま
// 渡していた——**FAR に文字の並びが出る**（データを番地として読んだ）。型の上では
// `String ≅ List(Char)` で同じものだが、**同型は型では無償、表現では有償**である（原理8）。
{
	const mix = "n0 : [~xs] ? ||xs ' 0||\nvia : ys ? n0 ys\nw : n0 [\\a , \\b]\nvia [`ab` , `cd`]";
	const ds = asm(mix).diagnostics;
	checkTrue(
		"要素の幅が食い違う呼び出しは名指しで断る",
		ds.some((d) => d.severity === "error" && /要素の幅が合いません/.test(d.message)),
		JSON.stringify(ds.map((d) => d.message))
	);
	// **断るのは食い違いだけである。** 器を渡すこと自体でも、要素が `{ptr, len}` である
	// ことでもない——片側だけなら合流は起きないので、どちらも診断ゼロで出る。
	check("器の並びだけなら診断は出ない", asm("n0 : [~xs] ? ||xs ' 0||\nvia : ys ? n0 ys\nvia [`ab` , `cd`]").diagnostics.length, 0);
	check("文字の並びだけなら診断は出ない", asm("n0 : [~xs] ? ||xs ' 0||\nn0 [\\a , \\b]").diagnostics.length, 0);
	// 変異は7本中5本が赤。素通りした2本は**等価**で、残す理由はどちらも「同じ事実を
	// 2か所で決めない」である：
	//   ・器の位置だけを見る条件（`p.w !== 2`）……… いまの資料では `cell` が付くのは
	//     2本で渡す位置だけなので外しても同じ。構造体（1本）の位置まで幅で比べ始めない
	//     ための条件なので残す。
	//   ・`String` の要素を `Char` と読む既定 ……… 束縛が要素型を持っているので今は要らない。
