...
function evaluate(node, env) {
  ...
  if (node.type === "block") {
    ...
    if (node.kind === "norm") {
      ...
      // ——下の「スカラーは1要素の器である」がそのまま当たる。
      //
      // 解釈器が番地を**参照セル**（`{__address__, get, set}` の物）で持っているのは
      // 実装の都合で、値そのものはスカラーである（機械の側では整数が1語）。ここはその
      // 物を見て下の「射は零射」へ落ち、`__` を返していた——**観測境界が意味に漏れた**形で、
      // 器を返す式が割れて見えるのと同じ1つの取り違えである。
      //
      // 実測の割れ（直す前。機械はいずれも 1）:
      //
      //   f : x ? ||$x||   に 42        解釈 __ ／ 機械 1
      //   f : x ? ||$x||   に (1,2,3)   解釈 __ ／ 機械 1   ← **器への番地でも 1**
      //   f : s ? ||$s||   に `abc`     解釈 __ ／ 機械 1
      //
      // **指す先を数えるのは `@` の仕事である。** `||@$x||` は前後とも 3 で、`{ptr,len}` の
      // 読みと矛盾しない。番地は指す先の長さを持たない——持っていたら `@` が要らない。
      if (inner && inner.__address__ === true) return 1;
